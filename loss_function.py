...
"""Lossfunction类中的forward方法接受三个参数:prediction(模型预测的位置和类别), targets(真实标签), priors_boxes(先验框)
返回位置损失loss_I和类别损失loss_c(其中类别损失使用交叉熵函数计算)"""
class LossFun(nn.Module):
    ...
    def forward(self, prediction,targets,priors_boxes):
        ...
        loss_loc = F.smooth_l1_loss(pre_loc_xij,tar_loc_xij,reduction='sum')
